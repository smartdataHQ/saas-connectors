...
- [X] T011 [P] Create `providers/cyclraccount/utils.go` with the same `isUUID` helper.
  ...
  **Cleanup 2026-09-15:** the Layer-2 amendment below established that account ids may be non-UUID strings, and neither helper had any remaining caller. Both unused `utils.go` files were removed to clear the repository lint gate. Existing id validation, endpoint routing, and error text are unchanged. Write dispatch was split into action/object helpers solely to meet the complexity limit; no capability or payload contract changed.
